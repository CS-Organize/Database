require("dotenv").config();
const express = require("express");
const app = express();

const { Client } = require("@notionhq/client");
const notion = new Client({ auth: process.env.NOTION_KEY });

// http://expressjs.com/en/starter/static-files.html
app.use(express.static("public"));
app.use(express.json()); // for parsing application/json

// http://expressjs.com/en/starter/basic-routing.html
app.get("/", async (req, res) => {
  res.sendFile(__dirname + "/views/index.html");
});

// Create new database. The page ID is set in the environment variables.
app.post("/databases", async (req, res) => {
  const pageId = process.env.NOTION_PAGE_ID;
  const title = req.body.dbName;
  console.log(pageId, title);

  try {
    const newDb = await notion.databases.create({
      parent: {
        type: "page_id",
        page_id: pageId,
      },
      title: [
        {
          type: "text",
          text: {
            content: title,
          },
        },
      ],
      properties: {
        Name: {
          title: {},
        },
      },
    });
    res.json({ message: "Database created", data: newDb });
  } catch (error) {
    res.json({ message: "Error creating database", error });
  }
});

// Create new page. The database ID is provided in the web form.
app.post("/pages", async (req, res) => {
  const { dbID, pageName, header } = req.body;

  try {
    const newPage = await notion.pages.create({
      parent: {
        type: "database_id",
        database_id: dbID,
      },
      properties: {
        Name: {
          title: [
            {
              type: "text",
              text: {
                content: pageName,
              },
            },
          ],
        },
      },
      children: [
        {
          object: "block",
          heading_2: {
            rich_text: [
              {
                text: {
                  content: header,
                },
              },
            ],
          },
        },
      ],
    });
    res.json({ message: "Page created", data: newPage });
  } catch (error) {
    res.json({ message: "Error creating page", error });
  }
});

// Create new block (page content). The page ID is provided in the web form.
app.post("/blocks", async (req, res) => {
  const { pageID, content } = req.body;
  console.log(pageID, content);

  try {
    const newBlock = await notion.blocks.children.append({
      block_id: pageID, // a block ID can be a page ID
      children: [
        {
          // Use a paragraph as a default but the form or request can be updated to allow for other block types: https://developers.notion.com/reference/block#keys
          paragraph: {
            rich_text: [
              {
                text: {
                  content: content,
                },
              },
            ],
          },
        },
      ],
    });
    res.json({ message: "Block created", data: newBlock });
  } catch (error) {
    res.json({ message: "Error creating block", error });
  }
});

// Create new page comments. The page ID is provided in the web form.
app.post("/comments", async (req, res) => {
  const { pageID, comment } = req.body;
  console.log(pageID, comment);

  try {
    const newComment = await notion.comments.create({
      parent: {
        page_id: pageID,
      },
      rich_text: [
        {
          text: {
            content: comment,
          },
        },
      ],
    });
    res.json({ message: "Comment created", data: newComment });
  } catch (error) {
    res.json({ message: "Error creating comment", error });
  }
});

// listen for requests :)
const listener = app.listen(process.env.PORT, () => {
  process.stdout.write("abdc");
  console.log(`Server is running and listening on http://localhost:${listener.address().port}`);
});
